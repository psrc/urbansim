...
class residential_units_of_useclass_SSS(Variable):
    """Number of residential units in buildings of the given use classification"""
    ...
    def dependencies(self):
        return ["_residential_units_of_useclass_%s =  where ( sanfrancisco.building.building_class_name=='%s', sanfrancisco.building.residential_units,0)" 
                % (self.useclass,self.useclass)]
 
    def compute(self,  dataset_pool):
        return self.get_dataset().get_attribute("_residential_units_of_useclass_%s" % self.useclass)
...
class Tests(opus_unittest.OpusTestCase):
    variable_name = 'sanfrancisco.building.residential_units_of_useclass_apts'
    ...
    def test_my_inputs(self):
        ...
        should_be = array([0, 12, 0, 0, 15, 16])
