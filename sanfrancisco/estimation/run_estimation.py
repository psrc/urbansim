...
class EstimationRunner(object):
    ...
    def run_estimation(self, estimation_config, model, save_estimation_results=True, diagnose=False):
        ...
        if type is None:
            exec("from sanfrancisco.estimation.%s_estimation_config import run_configuration" % model[0].lower())
            run_configuration = update_controller_by_specification_from_module(
                                run_configuration, model[1],
                                "sanfrancisco.estimation.%s" % self.spec_file)
        else:
            exec("from sanfrancisco.estimation.%s_estimation_config import %s_configuration as config" % (model[0].lower(),
                  model[0].lower()))
            ...
            run_configuration = conf.get_updated_configuration_from_module(
                                     run_configuration, "sanfrancisco.estimation.%s" % self.spec_file)
