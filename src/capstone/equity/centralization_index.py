...
class centralization_index(Variable):

  m_data = "number_of_low_income_households"
  t_data = "number_of_households"
  d_data = "travel_timehbw_am_drive_alone_to_cbd"
  a_data = "acres_of_land"

  def dependencies(self):
    return [attribute_label("gridcell", m_data),
            attribute_label("gridcell", t_data),
            attribute_label("gridcell", a_data),
            attribute_label("gridcell", d_data)]

  def compute(self, dataset_pool):
    min_pop = self.get_dataset('gridcell'.get_attribute(self.m_data).astype(float64)
    total_pop = self.get_dataset('gridcell'.get_attribute(self.t_data).astype(float64)
    min_prop = min_pop / total_pop
    dist = self.get_dataset('gridcell'.get_attribute(self.d_data).astype(float64)
    area = self.get_dataset('gridcell'.get_attribute(self.a_data).astype(float64)
    return calc_index(min_prop, dist, area)

	def calc_index(self, min_prop, dist, area):
		assert(min_prop.size == area.size)
		assert(dist.size == area.size)
