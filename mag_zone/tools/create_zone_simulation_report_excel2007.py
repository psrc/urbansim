...
from easygui import multenterbox, multchoicebox, choicebox, indexbox
import xlwt, os, sys
import win32com.client as win32

class RegionWideReport():
    
    def __init__(self, workbook, years, connection, run_name, base_year, column_counter=1):
        self.column_counter = column_counter
#        self.worksheet = workbook.add_sheet('region_wide_report')        
        self.worksheet = workbook.Worksheets.Add()
        self.worksheet.Name = 'region_wide_report'
        self.years = years
    ...
    def _print_years_to_workbook(self):
        self.worksheet.Cells(1,self.column_counter).Value = 'year'
        #self.worksheet.write(1,self.column_counter,'year')
        row_counter = 2
        for year in self.years:
            #self.worksheet.write(row_counter,self.column_counter,year)
            self.worksheet.Cells(row_counter,self.column_counter).Value = year
            row_counter += 1
...
#        self.worksheet.write(0,self.column_counter,'total_units')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_units'
        row_counter = 2
        for year in self.years:
...
#                self.worksheet.write(row_counter,self.column_counter,row[0])
                self.worksheet.Cells(row_counter,self.column_counter).Value = row[0]
                row_counter += 1
            r.close()
        self.column_counter += 1
...
#        self.worksheet.write(0,self.column_counter,'total_nonres_sqft')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_nonres_sqft'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'total_SF_units')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_SF_units'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'total_MF_units')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_MF_units'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'total_hh')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_hh'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'total_hh_SFR')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_hh_SFR'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'total_hh_MFR')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_hh_MFR'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'total_pop_SFR')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_pop_SFR'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'total_pop_MFR')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_pop_MFR'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'total_pop')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_pop'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'total_jobs')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_jobs'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'total_hh_income_0')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_hh_income_0'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter + 1,'total_hh_income_1')
        self.worksheet.Cells(1,self.column_counter+1).Value = 'total_hh_income_1'
        row_counter = 2
        for year in self.years:
...
#                self.worksheet.write(row_counter,self.column_counter + 1,row[0])
                self.worksheet.Cells(row_counter,self.column_counter+1).Value = row[0]
                row_counter += 1
            r.close()
        # households over 100k
#        self.worksheet.write(0,self.column_counter + 2,'total_hh_income_2')
        self.worksheet.Cells(1,self.column_counter+2).Value = 'total_hh_income_2'
        row_counter = 2
        for year in self.years:
...
#                self.worksheet.write(row_counter,self.column_counter + 2,row[0])
                self.worksheet.Cells(row_counter,self.column_counter+2).Value = row[0]
                row_counter += 1
            r.close()
...
#        self.worksheet.write(0,self.column_counter,'household_ct')
        self.worksheet.Cells(1,self.column_counter).Value = 'household_ct'
        row_counter = 2
...
#                self.worksheet.write(row_counter,self.column_counter,0)
                self.worksheet.Cells(row_counter,self.column_counter).Value = 0
                row_counter += 1
#                self.worksheet.write(row_counter,self.column_counter,row[1])
                self.worksheet.Cells(row_counter,self.column_counter).Value = row[1]
                row_counter += 1
            else:
#                self.worksheet.write(row_counter,self.column_counter,row[1])
                self.worksheet.Cells(row_counter,self.column_counter).Value = row[1]
...
#        self.worksheet.write(0,self.column_counter,'job_ct')
        self.worksheet.Cells(1,self.column_counter).Value = 'job_ct'
        row_counter = 2
...
#        self.worksheet.write(0,self.column_counter,'unplaced_hh')
        self.worksheet.Cells(1,self.column_counter).Value = 'unplaced_hh'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'unpl_hh_0_50k')
        self.worksheet.Cells(1,self.column_counter).Value = 'unpl_hh_0_50k'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter + 1,'unpl_hh_50_100k')
        self.worksheet.Cells(1,self.column_counter + 1).Value = 'unpl_hh_50_100k'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter + 2,'unpl_hh_100k_up')
        self.worksheet.Cells(1,self.column_counter + 2).Value = 'unpl_hh_100k_up'
        row_counter = 2
        for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'unplaced_jobs')
        self.worksheet.Cells(1,self.column_counter).Value = 'unplaced_jobs'
        row_counter = 2
        for year in self.years:
...
#            self.worksheet.write(0,column_counter,'unpl_job_sec%s' % sector)
            self.worksheet.Cells(1,self.column_counter).Value = 'unpl_job_sec%s' % sector
            for year in self.years:
...
#            self.worksheet.write(0,column_counter,'job_sec%s' % sector)
            self.worksheet.Cells(1,self.column_counter).Value = 'job_sec%s' % sector
            for year in self.years:
...
#        self.worksheet.write(0,self.column_counter,'total_job_spaces')
        self.worksheet.Cells(1,self.column_counter).Value = 'total_job_spaces'
        row_counter = 2           
        for year in self.years:
...
def get_total_jobs_by_year_and_subarea_building_type(subarea, years, building_type, workbook, run_name, base_year, connection):
    # Get total jobs by subarea and building typein a separate sheet
    # write column headings
#    worksheet = workbook.add_sheet('jobs_by_%s_yr_btype%s' % (subarea,building_type))
#    worksheet.write(0,0,'%s_id' % (subarea))
    worksheet = workbook.Worksheets.Add()
    worksheet.Name = 'jobs_by_%s_yr_btype%s' % (subarea,building_type)
    worksheet.Cells(1,1).Value = '%s_id' % (subarea)
    column_counter = 1
    for year in years:
#        worksheet.write(0,column_counter,'y'+str(year))
        worksheet.Cells(1,column_counter+1).Value = 'y'+str(year)
        column_counter += 1
...
#        worksheet.write(row_counter,0,row[0])
        worksheet.Cells(row_counter+1, 1).Value = row[0]
        row_counter += 1
    # get values and fill in table
    column_counter = 1
    for year in years:
        print('Computing total jobs in type %s building by %s for year %s' % (building_type, subarea, year))
        row_counter = 1
        ...
        query = '''select z.%s_id, count(*) as numjobs
                    ...
                    where b.building_type_id = %s
                    group by z.%s_id
                    order by z.%s_id''' % (subarea, year, run_name, year, run_name, year, run_name, year, building_type, subarea, subarea)
...
#            worksheet.write(row_counter,column_counter,row[0])
            worksheet.Cells(row_counter+1, column_counter+1).Value = row[0]
            row_counter += 1
        column_counter += 1
        r.close()
...
#    worksheet.write(0,0,'%s_id' % (subarea))
    worksheet = workbook.Worksheets.Add()
    worksheet.Name = 'jobs_by_%s_and_year' % (subarea)
    worksheet.Cells(1,1).Value = '%s_id' % (subarea)
    column_counter = 1
    for year in years:
#        worksheet.write(0,column_counter,'y'+str(year))
...
#    worksheet.write(0,0,'%s_id' % (subarea))
    worksheet = workbook.Worksheets.Add()
    worksheet.Name = 'hh_by_%s_and_year' % (subarea)
    worksheet.Cells(1,1).Value = '%s_id' % (subarea)
    column_counter = 1
    for year in years:
#        worksheet.write(0,column_counter,'y'+str(year))
...
#    worksheet.write(0,0,'%s_id' % (subarea))
    worksheet = workbook.Worksheets.Add()
    worksheet.Name = 'SFR_hh_by_%s_and_year' % (subarea)
    worksheet.Cells(1,1).Value = '%s_id' % (subarea)
    column_counter = 1
    for year in years:
#        worksheet.write(0,column_counter,'y'+str(year))
...
#    worksheet.write(0,0,'%s_id' % (subarea))
    worksheet = workbook.Worksheets.Add()
    worksheet.Name = 'MFR_hh_by_%s_and_year' % (subarea)
    worksheet.Cells(1,1).Value = '%s_id' % (subarea)
    column_counter = 1
    for year in years:
#        worksheet.write(0,column_counter,'y'+str(year))
...
#    worksheet.write(0,0,'%s_id' % (subarea))
    worksheet = workbook.Worksheets.Add()
    worksheet.Name = 'SFR_pop_by_%s_and_year' % (subarea)
    worksheet.Cells(1,1).Value = '%s_id' % (subarea)
    column_counter = 1
    for year in years:
#        worksheet.write(0,column_counter,'y'+str(year))
...
#    worksheet.write(0,0,'%s_id' % (subarea))
    worksheet = workbook.Worksheets.Add()
    worksheet.Name = 'MFR_pop_by_%s_and_year' % (subarea)
    worksheet.Cells(1,1).Value = '%s_id' % (subarea)
    column_counter = 1
    for year in years:
#        worksheet.write(0,column_counter,'y'+str(year))
...
#    worksheet.write(0,0,'%s_id' % (subarea))
    worksheet = workbook.Worksheets.Add()
    worksheet.Name = 'pop_by_%s_and_year' % (subarea)
    worksheet.Cells(1,1).Value = '%s_id' % (subarea)
    column_counter = 1
    for year in years:
#        worksheet.write(0,column_counter,'y'+str(year))
...
#    worksheet.write(0,0,'%s_id' % (subarea))
    worksheet = workbook.Worksheets.Add()
    worksheet.Name = 'res_units_by_%s_and_year' % (subarea)
    worksheet.Cells(1,1).Value = '%s_id' % (subarea)
    column_counter = 1
    for year in years:
#        worksheet.write(0,column_counter,'y'+str(year))
...
def get_total_DUs_SF_by_year_and_subarea(subarea, years, workbook, run_name, base_year, connection):
    ...
    #worksheet.write(0,0,'%s_id' % (subarea))
    worksheet = workbook.Worksheets.Add()
    worksheet.Name = 'sf_DUs_by_%s_and_year' % (subarea)
    worksheet.Cells(1,1).Value = '%s_id' % (subarea)
    column_counter = 1
    for year in years:
        #worksheet.write(0,column_counter,'y'+str(year))
...
#    worksheet.write(0,0,'%s_id' % (subarea))
    worksheet = workbook.Worksheets.Add()
    worksheet.Name = 'mf_DUs_by_%s_and_year' % (subarea)
    worksheet.Cells(1,1).Value = '%s_id' % (subarea)
    column_counter = 1
    for year in years:
#        worksheet.write(0,column_counter,'y'+str(year))
...
def get_total_nonres_sqft_by_year_and_subarea_building_type(subarea, years, building_type, workbook, run_name, base_year, connection):
    # Get total non-residential sqft by subarea in a separate sheet
    # write column headings
#    worksheet = workbook.add_sheet('nonres_sqft_by_%s_yr_btype%s' % (subarea, building_type))
#    worksheet.write(0,0,'%s_id' % (subarea))
    worksheet = workbook.Worksheets.Add()
    worksheet.Name = 'nonres_sqft_by_%s_yr_btype%s' % (subarea, building_type)
    worksheet.Cells(1,1).Value = '%s_id' % (subarea)
    column_counter = 1
    for year in years:
#        worksheet.write(0,column_counter,'y'+str(year))
...
#        worksheet.write(row_counter,0,row[0])
    ...
    column_counter = 1
    for year in years:
        print('Computing total non-residential sqft in building type %s by %s for year %s' % (building_type, subarea, year))
        row_counter = 1
        r = connection.execute("IF OBJECT_ID('tempdb..#distinct_%s','local') IS NOT NULL DROP TABLE #distinct_%s" % (subarea, subarea))
        r.close
        r = connection.execute('select distinct(%s_id) into #distinct_%s from %s_%s_zones order by %s_id' % (subarea, subarea, run_name, base_year, subarea))
        ...
        if year == base_year:
            query = '''select p.%s_id, sum(b.non_residential_sqft) as sum_non_res_sqft
                        ...
                        where b.building_type_id = %s
                        group by p.%s_id
                        order by p.%s_id''' % (subarea, year, run_name, year, run_name, year, building_type, subarea, subarea)
        else:
            query = '''select p.%s_id, sum(b.non_residential_sqft) as sum_non_res_sqft
                        into #non_res_sqft_%s
                        from %s_%s_buildings b
...
def save_excel_workbook(run_name, excel_output_path, workbook):
    ...
    try:
        excel_output_workbook_name = run_name + '.xlsx'
        if os.path.exists(os.path.join(excel_output_path,excel_output_workbook_name)):
            ...
            workbook.SaveAs(os.path.join(excel_output_path,excel_output_workbook_name))
        ...
        print('No worksheets have been created, workbook not saved.', end=' ') 
...
def get_available_building_type_ids(connection, run_name, lowest_year):
    # run query to get all columns
    query = "select building_type_id, building_type_name from %s_%s_buildingtypes order by building_type_id" % (run_name, lowest_year)
    r = connection.execute(query)
    # sort through columns for subarea id columns
    available_building_types = ['Skip']
    for row in r:
            available_building_types.append(str(row[0]) + ': ' + row[1])
    r.close()
    return available_building_types
...
def main():
    ...
    available_building_types = get_available_building_type_ids(connection, run_name, lowest_year)
    if available_building_types != []:
        chosen_building_type = indexbox('Choose a building type:','Available building types',available_building_types)
        if not chosen_building_type:
            chosen_building_type = ''
            print('No chosen building type.')
        else:
            print('chosen_building_type = %s' % (chosen_building_type))
    else:
        chosen_building_type = ''      
    # other setup variables
    query_fields = ['Base Year','Years','Run Name','Excel Output Path','Subarea', 'building_type_id']
    query_default_values = ['2009',chosen_years,run_name,'c:/working/excel_outputs',chosen_subarea, str(chosen_building_type)]
    ...
    building_type = query_variables[5]
    ...
    while True:
        ...
        if building_type != '':
            report_list.extend(
                        [ 'get_total_jobs_by_year_and_subarea_building_type'
                        , 'get_total_nonres_sqft_by_year_and_subarea_building_type'
                        ]
                               )        
        report_list.sort()
        ...
        for choice in choices:
            if 'building_type' in choice:
                building_type_choices.append(choice)
            else:
        ...
        for building_type_choice in building_type_choices:
            try:
                q = '%s(subarea, years, building_type,workbook, run_name, base_year, connection)' % building_type_choice
                exec(q)
            except exc.ProgrammingError as e:
                print('\nERROR when calling the following subarea report function: \n%s' % q)
                print('\nThere was a SQL error: \n%s\n' % (e))                
