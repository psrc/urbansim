...
class percent_high_income_households(Variable):
    ...
    high_income = "number_of_high_income_households"
    number_of_households = "number_of_households"
    
    def dependencies(self):
        return [attribute_label("zone", self.number_of_households, "mag_zone"), 
                attribute_label("zone", self.high_income, "mag_zone")]
        
    def compute(self, dataset_pool):
        return 100*safe_array_divide(self.get_dataset().get_attribute(self.high_income),
...
class Tests(opus_unittest.OpusTestCase):
    variable_name = "mag_zone.zone.percent_high_income_households"

    def test_my_inputs(self):
        high_income_hhs = array([8, 0, 35, 0])
        number_of_households = array([10, 20, 35, 0])
        
        values = VariableTestToolbox().compute_variable(self.variable_name, 
            {"zone":{ 
                "number_of_high_income_households":high_income_hhs, 
                "number_of_households":number_of_households}}, 
            dataset = "zone")
