...
from fazset.fazes import FazSet
...
class TestHLCModelEugene:
    ...
    def run(self, variables=[], debuglevel=0):
        ...
        return HouseholdLocationChoiceModelCreator().get_model(\
            ...
            sample_proportion_locations=None, debuglevel=debuglevel).run(\
                    specification, coefficients, gcs, hhs, \
                    agents_index=sample(range(77535),1000), \
...
def get_data_objects_from_DB(con, accessibilitiesflag=0, debuglevel=0):
    ...
    zones = ZoneSet(in_base = con, in_storage_type="MySQL", \
        accessibilitiesflag=accessibilitiesflag, \
        other_in_places=[PSRC_Settings.db1+".zones_in_faz_complete"], debuglevel=debuglevel)
...
class PSRC_Settings:
    ...
    db1 = "PSRC_HLC_dispersal_exp_scenario"
...
class PlotPSRCResults:
    def __init__(self, indicator, years):
        from rpy import r 
        r.postscript(file="PSRC_"+str(years)+"_years_zones_"+indicator+".ps")
#        r.par(mfrow=r.c(2,1))
        ...
        gcs30 = GridcellSet(in_base = PSRC_Settings.outputdir, in_storage_type="flt", \
                in_place="year_"+str(years)+"/" + PSRC_Settings.gcsubdir)
        gcs30.r_image(indicator, min_value=10000, max_value=150000, main="after "+str(years)+\
            " years (zones)")
        r.dev_off()
                
if __name__ == "__main__":
#    choices = TestHLCModelEugene().run(["residential_units_when_household_has_children", "cost_to_income_ratio"], debuglevel=3)
    sim = Run_PSRCsimulation(variables=[], indicators=[("gridcell","average_income"),\
            ("zone", "average_income")], \
            years=30, debuglevel=4)

#    PlotPSRCResults(indicator="aggregated_average_income", years=30)
