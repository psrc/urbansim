...
$rv = $dbh->do("drop table if exist $des_table")
...
$stm1 = "select distinct pin from $src_table";
...
while ($pin=$sth1->fetchrow()) {
	$stm2  = "insert into table $des_table"
	$stm2 .= "select * from $src_table where pin = $pin order by AREA DESC limit 1";
