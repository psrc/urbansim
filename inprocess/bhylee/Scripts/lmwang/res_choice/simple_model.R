...
pallvar <- read.delim("ex_ctht_pois.tab",na.strings = 'NULL')

for (i in 2:dim(pallvar)[2]) {
  pallvar <- pallvar[!is.na(pallvar[,i]),]
}
pallvar$ct.ht = as.factor(pallvar$ct_ht)
pallvar$income = as.factor(pallvar$income)
pallvar$age_range = as.factor(pallvar$age_range)
...
pfit.a <- glm(count ~ ct.ht + hhsize + income + linear.age + quadratic.age,
             data = pallvar, family = poisson)

pfit.b <- glm(count ~  (ct.ht + hhsize + income + linear.age + quadratic.age)^2,
             data = pallvar, family = poisson)

pfit.c <- glm(count ~ ct.ht + hhsize + income + age_range,
            data = pallvar, family = poisson)

pfit.c1 <- glm(count ~ ct.ht:hhsize + ct.ht:income + ct.ht:age_range,
            data = pallvar, family = poisson)

pfit.c2 <- glm(count ~  ct.ht:hhsize + ct.ht:income + ct.ht:linear.age + ct.ht:quadratic.age,
            data = pallvar, family = poisson)

pfit.d <- glm(count ~  (ct.ht + hhsize + income + age_range)^2,
            data = pallvar, family = poisson)

pfit.e <- glm(count ~  age_range + (ct.ht + hhsize + income + linear.age + quadratic.age)^2,
            data = pallvar, family = poisson)
