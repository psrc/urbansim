...
#
#    2. Data for year $year1, suffixed with "_z"; and
#
...
#         different, then the development type of the gridcell in $year1; these data are
#         suffexed with "_d";
#
#    4. Computed absolute differences for selected variables between $output_db data in $year1 
#         and WFRC_1997_output_2030_LRP data in $year1, suffixed with "_dl";
#
#    5. Computed percent-differences between $output_db data in $year1 and WFRC_1997_output_2030_LRP 
#         data in $year1, suffixed with "_Absolute_percent"; and
#
#    6. Computed percent-differences between $output_db growth data (from $base_year to $year1) and 
#         WFRC_1997_output_2030_LRP growth data (from $base_year to $year1), suffixed with 
#         "_Change_percent".
#
#
...
#    $base_year - the four-digit year of the base year, or the start of the simulation;
...
#tp  select year, count(*) as households from households_exported where year=$base_year or year=$year1 group by year;
#tp  select year, count(*) as jobs from jobs_exported where year=$base_year or year=$year1 group by year;
#tp  select year, sum(residential_units) as residential_units from gridcells_exported where year=$base_year or year=$year1 group by year;
#tp  select year, sum(commercial_sqft+industrial_sqft+governmental_sqft) as nonresidential_sqft 
#tp       from gridcells_exported where year=$base_year or year=$year1 group by year;
#tp  select year, min(HOME_ACCESS_TO_EMPLOYMENT_1), max(HOME_ACCESS_TO_EMPLOYMENT_1) 
#tp       from accessibilities where year=$base_year or year=$year1 group by year;
...
#tp  select year, count(*) as households from households_exported_endyears where year=$base_year or year=$year1 group by year;
...
#tp  select year, count(*) as jobs from jobs_exported_endyears where year=$base_year or year=$year1 group by year;
...
insert into gridcell_excess_capacity
     select 
          ...
          -1,-1
     from $baseyear_db.gridcell_capacity as gc, gridcells_exported as ge
     where gc.grid_id = ge.grid_id 
          AND ge.year=$base_year
     group by gc.grid_id;
create index gridcell_excess_capacity_grid_id 
     on gridcell_excess_capacity (grid_id);
update gridcell_excess_capacity as gec, $baseyear_db.gridcell_capacity as gc, gridcells_exported as ge
...
INSERT INTO gridcells_exported_endyears
     ...
     FROM gridcells_exported AS a,
          $baseyear_db.residential_units_for_home_based_jobs AS b,
          $baseyear_db.sqft_for_non_home_based_jobs AS c
     WHERE (a.year=$base_year OR a.year=$year1)
...
update gridcells_exported_endyears as a, $baseyear_db.gridcells AS b
...
update gridcells_exported_endyears as a, gridcell_excess_capacity AS gec
     ...
     where a.grid_id=gec.grid_id AND a.year=$base_year;
update gridcells_exported_endyears as a, gridcell_excess_capacity AS gec
     set a.excess_sqft=gec.excess_sqft_Z,
           a.excess_units=gec.excess_units_Z
     where a.grid_id=gec.grid_id AND a.year=$year1;


#tp  select year, sum(residential_units) as residential_units from gridcells_exported_endyears where year=$base_year or year=$year1 group by year;
#tp  select year, sum(commercial_sqft+industrial_sqft+governmental_sqft) as nonresidential_sqft 
#tp       from gridcells_exported_endyears where year=$base_year or year=$year1 group by year;
...
#tp  select year, min(HOME_ACCESS_TO_EMPLOYMENT_1), max(HOME_ACCESS_TO_EMPLOYMENT_1) 
#tp       from accessibilities_endyears where year=$base_year or year=$year1 group by year;
...
#tp  select year, count(*) as households from households_endyears where year=$base_year or year=$year1 group by year;
...
#tp select year, sum(households) from zones_hhdata where year=$base_year or year=$year1 group by year;
...
#tp  select year, count(*) as jobs from jobs_endyears where year=$base_year or year=$year1 group by year;
...
#tp  select year, sum(jobs) as jobs from zones_jobdata where year=$base_year or year=$year1 group by year;
...
CREATE TEMPORARY TABLE tmp_gridcell_fractions_in_zones
     SELECT * 
     FROM $baseyear_db.gridcell_fractions_in_zones;
...
CREATE TEMPORARY TABLE tmp_gridcell_fractions_whole
     ...
     FROM $baseyear_db.gridcells as a
...
#tp  select year, sum(residential_units) as residential_units from wedges_endyears where year=$base_year or year=$year1 group by year;
#tp  select year, sum(commercial_sqft+industrial_sqft+governmental_sqft) as nonresidential_sqft 
#tp       from wedges_endyears where year=$base_year or year=$year1 group by year;
#tp  select year, sum(home_based_job_spaces), sum(non_home_based_job_spaces), sum(total_job_spaces)
...
#tp  select year, sum(residential_units) as residential_units from zones_gcdata where year=$base_year or year=$year1 group by year;
#tp  select year, sum(commercial_sqft+industrial_sqft+governmental_sqft) as nonresidential_sqft 
#tp       from zones_gcdata where year=$base_year or year=$year1 group by year;
#tp  select year, sum(home_based_job_spaces), sum(non_home_based_job_spaces), sum(total_job_spaces)
...
#tp  select year, sum(dur) as residential_units from zones_endyears_short where year=$base_year or year=$year1 group by year;
#tp  select year, sum(sf_c+sf_i+sf_g) as nonresidential_sqft 
#tp       from zones_endyears_short where year=$base_year or year=$year1 group by year;
#tp  select year, sum(hhs) from zones_endyears_short where year=$base_year or year=$year1 group by year;
#tp  select year, sum(jobs) from zones_endyears_short where year=$base_year or year=$year1 group by year;
...
#tp select year, sum(households) from gridcells_hhdata where year=$base_year or year=$year1 group by year;
...
#tp select year, sum(jobs) from gridcells_jobdata where year=$base_year or year=$year1 group by year;
...
#tp  select year, sum(dur) as residential_units from gridcells_endyears_short where year=$base_year or year=$year1 group by year;
#tp  select year, sum(sf_c+sf_i+sf_g) as nonresidential_sqft 
#tp       from gridcells_endyears_short where year=$base_year or year=$year1 group by year;
#tp  select year, sum(hhs) from gridcells_endyears_short where year=$base_year or year=$year1 group by year;
#tp  select year, sum(jobs) from gridcells_endyears_short where year=$base_year or year=$year1 group by year;
...
DROP TABLE IF EXISTS zones_$year1;
CREATE TABLE zones_$year1 
     (zone_id int, county int, distlrg int, distmed int, distsml int);

ALTER TABLE zones_$year1
...
CREATE INDEX zones_$year1_zone_id
     ON zones_$year1 (zone_id);

INSERT INTO zones_$year1
...
FROM zones_endyears_short AS yr_a,
  ...
  $baseyear_db.zones AS zones
WHERE yr_a.year=$base_year AND
  yr_z.year=$year1 AND
  yr_a.zone_id=zones.zone_id AND
  yr_z.zone_id=zones.zone_id;

#tp  select sum(dur_a), sum(dur_z) from zones_$year1;
#tp  select sum(sf_c_a+sf_i_a+sf_g_a), sum(sf_c_z+sf_i_z+sf_g_z), sum(sf_tot_a), sum(sf_tot_z)
#tp       from zones_$year1;
#tp  select sum(hhs_a), sum(hhs_z) from zones_$year1;
#tp  select sum(jobs_a), sum(jobs_z) from zones_$year1;
#tp  select min(hae1_a), max(hae1_a), min(hae1_z), max(hae1_z) 
#tp       from zones_$year1;
#tp  select sum(js_hb_a), sum(js_nhb_a), sum(js_tot_a) from zones_$year1;
#tp  select sum(js_hb_z), sum(js_nhb_z), sum(js_tot_z) from zones_$year1;
...
UPDATE zones_$year1 AS a
     INNER JOIN WFRC_$base_year_output_$year1_LRP.zones_$year1 AS LRP
...
#tp  select year, sum(residential_units) as residential_units from gridcells_exported where year=$base_year or year=$year1 group by year;
#tp  select sum(dur_a), sum(dur_z), sum(dur_d) from zones_$year1;
#tp  select year, sum(commercial_sqft+industrial_sqft+governmental_sqft) as nonresidential_sqft 
#tp       from gridcells_exported where year=$base_year or year=$year1 group by year;
...
#tp       from zones_$year1;
#tp  select year, count(*) as households from households_exported where year=$base_year or year=$year1 group by year;
#tp  select sum(hhs_a), sum(hhs_z), sum(hhs_d) from zones_$year1;
#tp  select year, count(*) as jobs from jobs_exported where year=$base_year or year=$year1 group by year;
#tp  select sum(jobs_a), sum(jobs_z), sum(jobs_d) from zones_$year1;
#tp  select year, min(HOME_ACCESS_TO_EMPLOYMENT_1), max(HOME_ACCESS_TO_EMPLOYMENT_1) 
#tp       from accessibilities where year=$base_year or year=$year1 group by year;
...
#tp  select sum(js_hb_a), sum(js_nhb_a), sum(js_tot_a) from zones_$year1;
#tp  select sum(js_hb_z), sum(js_nhb_z), sum(js_tot_z) from zones_$year1;
#tp  select sum(js_hb_d), sum(js_nhb_d), sum(js_tot_d) from zones_$year1;

DROP TABLE IF EXISTS gridcells_$year1;
CREATE TABLE gridcells_$year1 (grid_id int, ci_id int, co_id int, 
...
CREATE INDEX gridcells_$year1_grid_id
     on gridcells_$year1 (grid_id);

INSERT INTO gridcells_$year1
...
UPDATE gridcells_$year1 AS a
     INNER JOIN WFRC_$base_year_output_$year1_LRP.gridcells_$year1 AS LRP
...
#tp  select year, sum(residential_units) as residential_units from gridcells_exported where year=$base_year or year=$year1 group by year;
#tp  select sum(dur_a), sum(dur_z) from gridcells_$year1;
#tp  select year, sum(commercial_sqft+industrial_sqft+governmental_sqft) as nonresidential_sqft 
#tp       from gridcells_exported where year=$base_year or year=$year1 group by year;
#tp  select sum(sf_c_a+sf_i_a+sf_g_a), sum(sf_c_z+sf_i_z+sf_g_z) 
#tp       from gridcells_$year1;
#tp  select year, count(*) as households from households_exported where year=$base_year or year=$year1 group by year;
#tp  select sum(hhs_a), sum(hhs_z) from gridcells_$year1;
#tp  select year, count(*) as jobs from jobs_exported where year=$base_year or year=$year1 group by year;
#tp  select sum(jobs_a), sum(jobs_z) from gridcells_$year1;

# Aggregate gridcell data to the city level

DROP TABLE IF EXISTS city_$year1;
CREATE TABLE city_$year1 (ci_id int, ci_na text);
     
ALTER TABLE city_$year1
  ADD COLUMN dur_a int, ADD COLUMN iv_r_a double, 
  ADD COLUMN sf_c_a double, ADD COLUMN iv_c_a double, ADD COLUMN sf_i_a double, ADD COLUMN iv_i_a double, ADD COLUMN sf_g_a double, 
  ADD COLUMN iv_g_a double, ADD COLUMN lv_r_a double, ADD COLUMN lv_nr_a double, ADD COLUMN sf_tot_a double, ADD COLUMN lv_tot_a double, 
  ADD COLUMN js_hb_a double, ADD COLUMN js_nhb_a double, ADD COLUMN js_tot_a double,
...
CREATE INDEX city_$year1_ci_id
     ON city_$year1 (ci_id);
   
INSERT INTO city_$year1
...
update city_$year1 AS a, $baseyear_db.cities AS b
     set a.ci_na = b.city_name
     where a.ci_id = b.city_id;

# Aggregate gridcell data to the county level

DROP TABLE IF EXISTS county_$year1;
CREATE TABLE county_$year1 (co_id int, co_na text);
     
ALTER TABLE county_$year1
  ADD COLUMN dur_a int, ADD COLUMN iv_r_a double, 
  ADD COLUMN sf_c_a double, ADD COLUMN iv_c_a double, ADD COLUMN sf_i_a double, ADD COLUMN iv_i_a double, ADD COLUMN sf_g_a double, 
  ADD COLUMN iv_g_a double, ADD COLUMN lv_r_a double, ADD COLUMN lv_nr_a double, ADD COLUMN sf_tot_a double, ADD COLUMN lv_tot_a double, 
  ADD COLUMN js_hb_a double, ADD COLUMN js_nhb_a double, ADD COLUMN js_tot_a double,
...
CREATE INDEX county_$year1_co_id
     ON county_$year1 (co_id);
   
INSERT INTO county_$year1
...
update county_$year1 AS a, $baseyear_db.counties AS b
     set a.co_na = b.county_name
     where a.co_id = b.county_id;

# Aggregate zone data to the small district level

DROP TABLE IF EXISTS distsml_$year1;
CREATE TABLE distsml_$year1 (distsml int, distmed int, distlrg int, county int);
     
ALTER TABLE distsml_$year1
  ADD COLUMN dur_a int, ADD COLUMN iv_r_a double, 
  ADD COLUMN sf_c_a double, ADD COLUMN iv_c_a double, ADD COLUMN sf_i_a double, ADD COLUMN iv_i_a double, ADD COLUMN sf_g_a double, 
  ADD COLUMN iv_g_a double, ADD COLUMN lv_r_a double, ADD COLUMN lv_nr_a double, ADD COLUMN sf_tot_a double, ADD COLUMN lv_tot_a double, 
  ADD COLUMN js_hb_a double, ADD COLUMN js_nhb_a double, ADD COLUMN js_tot_a double,
...
CREATE INDEX distsml_$year1_distsml
     ON distsml_$year1 (distsml);
     
INSERT INTO distsml_$year1
...
UPDATE distsml_$year1 AS a
     INNER JOIN WFRC_$base_year_output_$year1_LRP.distsml_$year1 AS LRP
...
CREATE TABLE distsml_comparisons
     SELECT a.distsml AS distsml,
        (LN(a.dur_z/a.dur_a))/($year1 - $base_year) AS dur_aarc_u,
        (LN(b.census_residential_units_2000/b.census_residential_units_1990))/10 AS dur_aarc_c,
        (LN(a.hhs_z/a.hhs_a))/($year1 - $base_year) AS hhs_aarc_u,
        (LN(b.census_households_2000/b.census_households_1990))/10 AS hhs_aarc_c, 
        (LN(a.j_3p_z/a.j_3p_a))/($year1 - $base_year) AS jobs_aarc_u,
        (LN(b.census_employment_2001/b.census_employment_1990))/10 AS jobs_aarc_c,
        (LN(a.pop_z/a.pop_a))/($year1 - $base_year) AS pop_aarc_u,
        (LN(b.census_population_2000/b.census_population_1990))/10 AS pop_aarc_c,
        (a.dur_z-b.old_method_residential_units_$year1) AS adpt_dif_DUR,
        (a.hhs_z-b.old_method_households_$year1) AS adpt_dif_HH,
        (a.j_3p_z-b.old_method_employment_$year1) AS adpt_dif_EMP,
        (a.pop_z-b.old_method_population_$year1) AS adpt_dif_POP,
        (a.dur_z-b.old_method_residential_units_$year1)/a.dur_z AS adpt_pct_dif_DUR,
        (a.hhs_z-b.old_method_households_$year1)/a.hhs_z AS adpt_pct_dif_HH,
        (a.j_3p_z-b.old_method_employment_$year1)/a.j_3p_z AS adpt_pct_dif_EMP,
        (a.pop_z-b.old_method_population_$year1)/a.pop_z AS adpt_pct_dif_POP   
     FROM distsml_$year1 AS a 
     INNER JOIN $baseyear_db.small_districts AS b
