...
# This script selects, for each parcel with NULL land use, the land use
...
#		Required fields: PARCEL_ID, X_COORD, Y_COORD, LAND_USE, IMPUTE_FLAG.
#		Note: X_COORD and Y_COORD must be non-null.
# OUTPUT:
# 	Writes to the input file, changing LAND_USE to a value selected at random
...
#my $neighborhood_distance = 500;
 ...
 my $db = <STDIN>;
 chomp $db;
#my $db = "land_use_imputation_cpeak";
...
$qry_make_nullstable .= " SELECT PARCEL_ID, X_COORD, Y_COORD, LAND_USE FROM $parceltable WHERE $parceltable";
$qry_make_nullstable .= ".LAND_USE IS NULL";
...
$qry_make_valstable .= " SELECT PARCEL_ID, X_COORD, Y_COORD, LAND_USE FROM $parceltable WHERE $parceltable";
$qry_make_valstable .= ".LAND_USE IS NOT NULL AND IMPUTE_FLAG IS NULL";
...
########
# Get min and max x and y for the parcels without LAND_USE
...
##  Then impute P's LAND_USE by randomly selecting one of the land_uses from the selected records from $valstable.
...
while (my $nullrow = $select_sth1->fetchrow_arrayref) {
	...
	my $qry_neighbors = "SELECT PARCEL_ID, LAND_USE FROM $valstable WHERE X_COORD >= $nbr_min_x";
	...
	####### import the neihborhood parcels and their LAND_USEs into two arrays @nbr_pin and @nbr_land_use
	while (my $neighbor = $sth_neighbors->fetchrow_arrayref) {
		#push (@nbr_pin, @$neighbor[0]);
		push (@nbr_land_use, @$neighbor[1]);
		#print @$neighbor[1], "\n";
	}

	####### print a summary of the land_uses within the neighborhood.
	#my $qry_summary = "SELECT COUNT(*) AS CNT, LAND_USE FROM $valstable WHERE X_COORD >= $nbr_min_x";
	#$qry_summary .= " AND X_COORD <= $nbr_max_x AND Y_COORD >= $nbr_min_y AND Y_COORD <= $nbr_max_y";
	#$qry_summary .= " GROUP BY LAND_USE";
	...
	my $max_nbr_ref = $#nbr_land_use + 1; #the number of parcels in $null_pin's neighborhood.
	####### If there are any parcels in the neighborhood of parcel $null_pin,
	#######  draw a random integer $selected_nbr_index between 0 and the highest index in @nbr_land_use, and use this
	#######  as the index by which to select land_use from the neighborhing parcels.
	if ($max_nbr_ref == 0) {
		print "There are no neighboring parcels for PARCEL_ID $null_pin. \n";
		print "LAND_USE for PARCEL_ID $null_pin not changed. \n";
	} else {
		my $selected_nbr_index = int(rand $max_nbr_ref); # Get a random # between 0 and the # of neighboring parcels
		my $imputed_land_use = $nbr_land_use[$selected_nbr_index];
		# print $nbr_pin[$selected_nbr_index], "\t $nbr_land_use[$selected_nbr_index] \t $selected_nbr_index \n";
		####### Slect $nbr_land_use[rndm] as the land use to ascribe to $nullparcel.
		# For using of integer type LAND_USE field
		#my $qry_impute = "UPDATE $parceltable SET LAND_USE = \"$imputed_land_use\" WHERE PARCEL_ID = \"$null_pin\" ";
		my $qry_impute = "UPDATE $parceltable SET LAND_USE = $imputed_land_use WHERE PARCEL_ID = \"$null_pin\" ";
		#print $qry_impute, "\n";
		print "Imputing LAND_USE for PARCEL_ID $null_pin to \"$imputed_land_use\".\n";
	...
	@nbr_land_use = ();  #Clear the list of possible land_uses.
...
#my $qry_rectify = "UPDATE $parceltable SET LAND_USE = NULL WHERE PARCEL_ID = \"9999999999\"";
