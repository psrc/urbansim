...
#
#    WFRC_1997_output_2030_LRP - the name of the database containing the UrbanSim output tables;
...
insert into tmp_table1(YEAR, JOB_SPACES)
     ...
     from gridcells_exported a, WFRC_1997_baseyear.residential_units_for_home_based_jobs b
...
insert into tmp_table2(YEAR, JOB_SPACES)
     ...
     from gridcells_exported a, 
          WFRC_1997_baseyear.sqft_for_non_home_based_jobs b
     where a.DEVELOPMENT_TYPE_ID=b.DEVELOPMENT_TYPE_ID
     group by a.YEAR;
...
create table jobs_3plus_per_year 
     ...
     from WFRC_1997_baseyear.annual_employment_control_totals 
     where sector_id>=3 
     group by year;
...
create table nonresidential_sqft_per_year
     select YEAR, 
          sum(COMMERCIAL_SQFT+INDUSTRIAL_SQFT+GOVERNMENTAL_SQFT)/1000 as NONRESIDENTIAL_SQFT
