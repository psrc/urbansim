...
class CantConnectToDatabaseException(Exception):
    """ Exception thrown when some Sql instance that the code requires is unreachable.  
        
        A common reason for this is that your environment variables that pertain are not set
        properly. For example, for MySql, MYSQLUSERNAME and MYSQLPASSWORD are incorrectly set (for this 
        MySql instance).              
    ...
    def __str__(self):
        return ("Unable to connect to some particular instance of Sql.  "
                "A common reason for this is that your environment variables that pertain are not set "
                "properly. For example, for MySql, MYSQLUSERNAME and MYSQLPASSWORD are incorrectly set (for this "
                "MySql instance). "
