...
from numpy import where, float32, ones, put, float64, fromfile
...
def _create_flt_file(file_name, data, flat_indices, offset):
    if os.path.exists(file_name):
        #os.remove(file_name)
        content = fromfile(file_name, dtype=float32)
        put(content.ravel(), flat_indices[offset:], data[offset:])
    else:
        content = nodata_values*ones(shape=(nrows, ncols)).astype(float32)
        put(content.ravel(), flat_indices, data)
