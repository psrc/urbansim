...
class EstablishmentAllocationModel(Model):
    """Allocates establishments from submarket to building.
    """
    model_name = "Establishment Allocation Model"
    ...
    def run(self, agents_index=None):
        ...
        def building_match(idx_estabs, idx_blds):        
            ...
            resources = Resources({"capacity":building_set['vacant_units'][idx_blds], "agent_units":establishment_set['employees'][idx_estabs], "lottery_max_iterations":50})
            choices = lottery_choices().run(probabilities, resources=resources)
            counter = 0
            for choice in choices:
                if choice == -1:
                    establishment_set['building_id'][idx_estabs[counter]] = -1
                else:
                    establishment_set['building_id'][idx_estabs[counter]] = building_set['building_id'][idx_blds[choice]] 
                counter += 1        
        ...
        establishment_set = dataset_pool.get_dataset('establishment')

        building_set.add_attribute(name='employment_submarket_id', data=building_set.compute_variables('drcog.building.employment_submarket_id'))
        building_set.add_primary_attribute(name='vacant_units', data=building_set.compute_variables('building.non_residential_sqft/250 - building.aggregate(establishment.employees)'))
        
        agents_esubmarkets = establishment_set.get_attribute('employment_submarket_id')
        building_esubmarkets = building_set.get_attribute('employment_submarket_id')
        
        estab_building_ids = establishment_set.get_attribute('building_id')
        
        esubmarkets = np.unique(agents_esubmarkets[agents_index])
        
        cond_array = np.zeros(establishment_set.size(), dtype="bool8")
        ...
        for employment_submarket_id in esubmarkets:
            ...
            if (index_estab.size > 0) and (index_bld.size > 1):
                ...
                establishment_set['building_id'][index_estab] = np.array(index_estab.size*[bld_id])
            else:
        ...
        agents_index = np.where(establishment_set['building_id']==-1)
        esubmarkets = np.unique(agents_esubmarkets[agents_index])
        cond_array = np.zeros(establishment_set.size(), dtype="bool8")
        cond_array[agents_index] = True
