...
class RealEstateTransitionModelBySubmarkets(RealEstateTransitionModel):
    ...
    def run(self):
        ...
        #for zone in zones, create index of buildings in zone, create subset of buildings table using index, add to dataset pool, run RETM, place buildings within zone, remove subset table from dataset pool
        in_zone = building_set.compute_variables('_in_zone = building.disaggregate(parcel.zone_id)==2')
        ...
        logger.log_status("table_data: %s" % (table_data))
        ...
        dataset_pool.add_datasets_if_not_included({
                                            "buildings_zone":buildings_zone})
        logger.log_status("Number of buildings: %s" % (buildings_zone.size()))
        
        #remove dataset from pool at end
        dptm = RealEstateTransitionModel(target_vancy_dataset=dataset_pool.get_dataset('target_vacancy'))
        results, index = dptm.run(realestate_dataset = buildings_zone,#dataset_pool.get_dataset('building'),
                           year = current_year,
        ...
        #del results['development_project_id']
        attribs2 = results.get_known_attribute_names()
        table_data2={}
        for name in attribs2:
            logger.log_status("attrib: %s" % (name)) 
            logger.log_status("attrib: %s" % (results.get_attribute("%s" %(name)) ))
            if name in attribs:
                table_data2["%s"%(name)]=results.get_attribute("%s" %(name))
        logger.log_status("table_data2: %s" % (table_data2)) 
        building_set.add_elements(data=table_data2, require_all_attributes=False, change_ids_if_not_unique=True) 
        ##########Ok, at this point, I can loop through zones and add all sampled buildings to the buildings table with parcel_id of -1.  At this point, move to placing the building within the appropriate zone, similar to how its done below
        #####Now connect the two parts of the code!!  My zonal RETM implementation above, with my parcel-placement code below.  Tweak the parcel placement code so that it works at the zone level too.
        ...
        # for building in index_new_commercial_units:
            ...
            # parcel_id_to_assign = parcel_ids_with_enough_capacity[:1] #WHAT TO DO ONCE THERE ARE NO PARCELS WITH CAPACITY?!?
