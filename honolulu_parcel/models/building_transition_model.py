...
#             building_use_table,
        ...
        unit_attributes = building_use_classification_table.get_attribute('unit_name')
        unit_attributes_by_type = building_use_classification_table.get_attribute('units')
...
#            type_code = building_types_table.get_id_attribute()[itype]
            ...
            logger.log_status("vacancy table get vacancy attribute: %s" % (vacancy_table.get_attribute(vacancy_attribute)))
            logger.log_status("vacancy table get year attribute: %s" % (vacancy_table.get_attribute('year')))
            logger.log_status("year: %s" % (year))
            logger.log_status("id attribute: %s" % (vacancy_table.get_id_attribute()))
            year_idx = year-2000
            #target_vacancy_rate = vacancy_table.get_data_element_by_id(year).vacancy_attribute
            target_vacancy_rate = eval("vacancy_table.get_data_element_by_id(%s).%s" % (year_idx,vacancy_attribute))
            logger.log_status("target_vacancy_rate: %s" % (target_vacancy_rate))

            compute_resources = Resources(resources)
            compute_resources.merge({"debug":self.debug})
            units_attribute = unit_attributes[itype]
            units_type_attribute = unit_attributes_by_type[itype]
            total_units_attribute = 'total_' + units_type_attribute
            vacant_units_attribute = 'vacant_' + units_type_attribute

            # determine current-year vacancy rates
            building_set.compute_variables("honolulu_parcel.building." + vacant_units_attribute,
                                           resources = compute_resources)

            building_set.compute_variables("honolulu_parcel.building." + total_units_attribute,
                                           resources = compute_resources)

            vacant_units_sum = building_set.get_attribute(vacant_units_attribute).sum()
            units_sum = float( building_set.get_attribute(total_units_attribute).sum() )
...
#                    location_set.get_attribute(type+"_improvement_value" ).sum(), units_sum)
            ...
            history_type = history_table.get_attribute("building_type_id")
