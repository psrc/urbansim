...
class total_home_based_job_space(Variable):
    ...
    def dependencies(self):
        return ["sum_minimum_persons_and_2=employment_submarket.aggregate(urbansim_parcel.household.minimum_persons_and_2)",
                "honolulu_parcel.employment_submarket.is_multi_family_residential",
                "emp_submarket_resunits=employment_submarket.aggregate(building.residential_units)"]
                
    def compute(self,  dataset_pool):
        emp_submarkets = self.get_dataset()
        job_space = emp_submarkets.get_attribute("sum_minimum_persons_and_2").copy()
        is_mf = emp_submarkets.get_attribute("is_multi_family_residential").astype("bool8")
        job_space[is_mf] = minimum(30, emp_submarkets.get_attribute("emp_submarket_resunits")[is_mf])
        return job_space
