...
class income_and_ln_housing_value(Variable):
    ''' income * ln_housing_value'''
    _return_type='float32'
    hh_income = 'income'
    
    def dependencies(self):
        return ['parcel_ln_housing_value = ln(psrc.parcel.housing_value)',
                attribute_label('household', self.hh_income)]

    def compute(self, dataset_pool):
        return self.get_dataset().multiply(self.hh_income, 
                        'parcel_ln_housing_value')
...
class Tests(opus_unittest.OpusTestCase):
    variable_name = 'psrc.household_x_parcel.income_and_ln_housing_value'
