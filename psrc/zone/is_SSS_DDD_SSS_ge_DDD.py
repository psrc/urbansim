...
class is_SSS_DDD_SSS_ge_DDD(Variable):
...
class Tests(opus_unittest.OpusTestCase):
    def get_values(self, min, number):
        self.variable_name = "psrc.zone.is_employment_within_%s_minutes_travel_time_hbw_am_transit_walk_ge_%s" % (min, number)
    ...
    def test_to_2_ge_9(self):
        values = self.get_values(2, 9)
        should_be = array([0, 1])
        self.assert_(ma.allequal(values, should_be), 
                     msg="Error in " + self.variable_name)

    def test_to_4_ge_11(self):
