...
class population_per_acre(Variable):
    """The population of the zone / land acres in the zone, rounded to the nearest integer. """
    _return_type = "int32"
    ...
    def compute(self, dataset_pool):
        population_per_zone = self.get_dataset().sum_dataset_over_ids(dataset_pool.get_dataset('gridcell'),
                                                                      self.gc_population)
        acres_per_zone = self.get_dataset().sum_dataset_over_ids(dataset_pool.get_dataset('gridcell'),
                                                                 self.acres_of_land)
        result = (population_per_zone / acres_per_zone.astype("float32")).round()
        result[population_per_zone == 0] = 0
        return result
...
class Tests(opus_unittest.OpusTestCase):
    variable_name = "psrc.zone.population_per_acre"
    ...
    def test_my_inputs(self):
        ...
        should_be = array([400/(2*105.0), 200/(105.0/2), 400/105.0, inf, 0.0]).round()
