...
class Tests(opus_unittest.OpusTestCase):
    ...
    def test_versioning_with_aggregate(self):
        ...
        self.assertTrue(ds2.get_version("my_var")==0)
        ds2.compute_variables([variable], dataset_pool=dataset_pool)
        self.assertTrue(ds2.get_version("my_var")==0) # version should stay the same, i.e. it should not recompute
        ds0.touch_attribute("id1") # has version 2
        ds2.compute_variables([variable], dataset_pool=dataset_pool)
        self.assertTrue(ds2.get_version("my_var")==1) # version should be 1, i.e. it should recompute when id changes
        ds1.touch_attribute("id2") # has version 1
        ds2.compute_variables([variable], dataset_pool=dataset_pool)
        self.assertTrue(ds2.get_version("my_var")==2) # version should be 2, i.e. it should recompute when id changes
    ...
    def test_versioning_with_disaggregate(self):
        ...
        ds.compute_variables([var], dataset_pool=dataset_pool)
        self.assertTrue(ds.get_version("my_var")==0) # version should stay the same, i.e. it should not recompute
        ds.touch_attribute("id2") # has version 2
        ds.compute_variables([var], dataset_pool=dataset_pool)
        self.assertTrue(ds.get_version("my_var")==1) # version should be 1, i.e. it should recompute when id changes
