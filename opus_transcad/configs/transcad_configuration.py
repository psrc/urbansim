...
class TransCADConfiguration(Configuration):
    def __init__(self, scenario_dir_name, mode='full', years_to_run=None):
		...
		travel_model_configuration['travel_model_base_directory'] = scenario_dir_name
		...
		#in format of dictionary with key being the full qualified urbansim variable name and value the transcad column name
		#urbansim variable name is the full qualified urbansim variable name, most likely for taz
		#transcad column name is the column name in transcad data table, which are updated by data from UrbanSim cache
		...
		#this specifies data from transcad to urbansim in  
		#format: {matrix name: {transcad variable name: urbansim variable name} }
		#with additional keys for #row_index name and column_index name 
		...
		tm_to_urbansim_variable_mapping = {
			 "row_index_name":"ZoneID",
			 "col_index_name":"ZoneID",
			 "AMHwySkims": {
		...
		travel_model_configuration['macro'] = {
		   ...
		   'get_cache_data_into_transcad': 'SEMCOGImportTabFile', # macroname in travel_model_configuration['ui_file'],
		   'get_transcad_data_into_cache': 'SEMCOGExportMatrices',  # macroname in  travel_model_configuration['ui_file'],
		   'run_semcog_travel_model': 'SEMCOG Run Loops',  # macroname in  travel_model_configuration['ui_file'],
		   'get_file_location': 'SEMCOGGetFileLocation', # macroname in travel_model_configuration['ui_file']
		}
