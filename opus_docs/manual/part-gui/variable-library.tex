...
area: \variable{ln(gridcell.population_density)}.  Note that the dialog box
...
\variable{ln_population_density} variable.  More generally, a new
...
\variable{gridcell.population_density}.  Or our new variable can be used
...
variables or subexpressions.  The available functions include \code{ln},
\code{exp}, and \code{sqrt}.  Also, we can use the operators from the numpy
package, including \code{+ - * / **} (where \code{ **} is exponentiation).
Again, the expressions are all array-valued, so that 
\variable{ln(2*gridcell.population_density)} is an array, 
