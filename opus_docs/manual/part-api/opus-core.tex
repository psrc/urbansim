...
plotted for example as a histogram\histogramindex
or as a scatter plot.\scatterplotindex
...
\item[\method{r_histogram(name, ...)}] \rindex creates
such histogram including a density line using the rpy \rpyindex library.
\item[\method{plot_scatter(name_x, name_y, ...)}] \rindex creates a scatter plot for two
attributes \attributesindex using matplotlib. \matplotlibindex
\item[\method{r_scatter(name_x, name_y, ...)}] \rindex creates a scatter plot
...
\index{class!InteractionDataset}
%
...
\label{sec:chunk-model} \index{class!ChunkModel}
...
\index{class!JoinAttributeModificationModel}
%
...
\class{Utilities} class implemented, called {\bf linear_utilities}, \index{class!Utilities!linear_utilities}
\index{utilities!linear utilities}
...
A child class of \class{linear_utilities}, called {\bf linear_utilities_diagnose}, 
\index{class!Utilities!linear_utilities_diagnose} serves analyzing changes in utilities 
...
\index{class!Probabilities} \index{probabilities}
...
\index{class!Regression}\index{regression}
...
\index{class!EstimationProcedure}\index{estimation}
%
...
\verb|standard_errors| contain arrays of estimated coefficients \coefficientsindex and their
...
\index{Specification}
\index{class!EquationSpecification}\index{model!specification}
%
...
the same length as \verb|variables|. \variablesindex It is interpreted as the $i$-th variable \variablesindex
...
\item[variables] - if this argument is given, it serves as a filter for the
  variables \variablesindex loaded from the storage.
\end{description}
For each of the class properties variables, \variablesindex coefficients, \coefficientsindex equations,
...
\variablesindex \verb|'coefficient_name'|, \coefficientsindex
...
\index{class!Coefficients}\index{model!coefficients}
%
...
entries 'field_coefficient_name', \coefficientsindex 'field_estimate',
'field_standard_error', and 'field_submodel_id', respectively. Default
values for the column names are \verb|'variable_name'|, \variablesindex \verb|'coefficient_name'|, \coefficientsindex 
...
\index{dataset pool}\index{class!DatasetPool}
%
...
\index{model group}\index{class!ModelGroup}
%
...
The class \class{ModelGroupMember}\index{class!ModelGroupMember}\index{model group member} is initialized by
\begin{description}
...
\begin{description}
\item[GeneralResources] - \index{GeneralResources} \index{class!GeneralResources} it is a python \pythonindex dictionary with some additional methods.
\item[SessionConfiguration] - \index{SessionConfiguration} \index{class!SessionConfiguration} a singleton (child of \class{GeneralResources} that is to be configured
        ...
        creates.
\item[Configuration] - \index{Configuration}  \index{class!Configuration} 
...
\item[Resources] - \index{Resources} \index{class!Resources} it is a child of \class{Configuration}. It has access to the
\class{SessionConfiguration}.
\item[ChunkSpecification] - \index{ChunkSpecification} \index{class!ChunkSpecification} class for configuring chunks used by the \class{ChunkModel} (see 
