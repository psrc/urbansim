...
\item[Expression] (e.g.\ \code{ln(urbansim.gridcell.population+1)})
  ...
  operators --- see Sections \ref{sec:expressions} and
  \ref{sec:implementation-of-expressions}.
...
\subsection{Using Expressions to Create Opus Variables}
...
(Section \ref{sec:expressions}).  Each newly-encountered expression is compiled into
...
\begin{description}
\item[GeneralResources] - \index{GeneralResources} \index{GeneralResources@\class{GeneralResources} class} it is a python \pythonindex dictionary with some additional methods.
