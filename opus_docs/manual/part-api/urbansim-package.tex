...
\item[control_totals] - an instance of \class{HousehldControlTotalDataset}. It must have at least attributes \attributesindex
...
\item[control_totals] - an instance of \class{EmploymentControlTotalDataset}. 
  It must have at least attributes \attributesindex ``sector_id'',
...
of the building types are home based (see ``job_building_types'' in~\ref{sec:employment-tables}).
 \item[data_objects] - a dictionary containing other datasets \datasetindex and arguments
  needed for computing variables. Default is None. \variablesindex
\item[resources] - additional \class{Resources} for controlling the
  simulation run. Default is None. The argument is not used in this version of the model. \modelsindex
\end{description}
...
%
{\it Input:}\\[1mm]
In addition to its parents' arguments {\bf specification}, {\bf coefficients}, \coefficientsindex {\bf agent_set}, {\bf agents_index},
...
%
This model (class \class{HouseholdLocationChoiceModel}) is an Agent Location Choice Model that makes default settings
...
\item[agent_set] - A \class{HouseholdDataset} object used in Household Relocation
  Model. \modelsindex
\item[agents_index] - Results of Household Relocation Model. \modelsindex Make sure that model is turned on 
...
The class is initialized by passing the following arguments:
\begin{description}
\item[group_member] - An object of class \class{ModelGroupMember} (see Section~\ref{sec:model-group}) 
...
\item[agents_index] - Results of Employment Relocation Model \modelsindex. Make sure that model is turned on 
...
\item[dataset_pool] - A pool of dataset used for computing variables. Default is None.
\end{description}
