...
\item \code{hwy_300 = gridcell.distance_to_highway<300} generates a dummy
...
\item \code{ln_pop = ln(urbansim.gridcell.population} computes the log of
  an existing variable, \code{population}, which is in the
  \package{urbansim} package and applies to the gridcell dataset.

\item \code{pop_emp_ratio = urbansim.gridcell.population /
  urbansim.gridcell.number_of_jobs} computes the ratio of population to
...
\code{population = zone.aggregate(urbansim.gridcell.population)}
...
\code{population = parcel.aggregate(household.persons, intermediates = [building])}
...
component \file{household} set has an attribute \verb|income|, and
the \file{gridcell}
