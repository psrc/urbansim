...
\usepackage{amsmath, graphics, graphicx, rotating, html, upquote, makeidx}
\usepackage{color, fancyhdr, longtable, epsfig,lscape, booktabs,multirow, boxit, listings}
...
\input{introduction}
\input{urbansim-overview}
\input{opus-overview}

\part{Opus and UrbanSim User Guide}\label{part-user-guide}
\input{installation}
\input{python-numpy-expressions}
\input{creating-models}
%\input{using-urbansim}
%\input{indicators}

\part{Opus and UrbanSim Reference Guide}\label{part-reference-guide}
%\input{urbansim-package-tutorial}
\input{opus-core}
\input{urbansim-package}
%\input{urbansim-database-tables}

\part{Information for Software Developers}\label{part-developers}
\input{programming}
\input{writing-documentation}

\appendix
\part{Appendices}
\input{opus-core-selected-methods}
