% $Id: urbansim-database-tables.tex,v 1.60 2007/06/01 23:49:22 borning Exp $

% Copyright (c) 2005 Center for Urban Simulation and Policy Analysis,
...
variable_name \variablesindex & varchar & \\\hline 
coefficient_name \coefficientsindex & varchar & \\\hline
...
In the prose below, we use the following example: let R1-R3 be in the
...
See above. Each row defines one DevelopmentType.
...
\begin{itemize}
\tight
\item development_type_id must be unique and greater than zero.
\item We recommend that development_type_ids start at 1 and be sequential.
...
\section{Database Tables about Events}

The change_typeS in these events are:
\begin{itemize} \tight
\item ``A'' for Add
\item ``N'' for No Change
\item ``R'' for Replace
\item ``D'' for Delete
...
that addition of the specified number of sqft, residential units, and
...
\item scheduled_year must be between absolute_min_year and absolute_max_year
\item residential_units must be greater than or equal to zero and \textless{}=
absolute_max_cell_residential_units
\item commercial_sqft must be greater than or equal to zero and \textless{}=
absolute_max_cell_sqft
\item industrial_sqft must be greater than or equal to zero and \textless{}=
absolute_max_cell_sqft
\item governmental_sqft must be greater than or equal to zero and \textless{}=
absolute_max_cell_sqft
\item residential_improvement_value must be greater than or equal to zero and
\textless{}= absolute_max_cell_dollars
\item commercial_improvement_value must be greater than or equal to zeroand
\textless{}= absolute_max_cell_dollars
\item industrial_improvement_value must be greater than or equal to zero and
\textless{}= absolute_max_cell_dollars
\item governmental_improvement_value must be greater than or equal to zero and
\textless{}= absolute_max_cell_dollars
...
\item governmental_improvement_value must be greater than or equal to zero and
\textless{}= absolute_max_cell_dollars
\item starting_development_type_id is ignored on input, and updated (to be the
ending development type for that gridcell in that year) as the developer model
estimation data writer runs. It must \emph{not} be a member of the
...
\\
\hline
cars &Number of cars in household; must be 0, 1, 2, or 3+ & \emph{user-configurable}
\\
\hline
