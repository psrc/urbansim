...
\chapter{Tutorial for the \package{urbansim} Opus Package}
\label{urbansim-tutorial}

\index{tutorial!\package{urbansim}}
This chapter gives a tutorial on using \package{urbansim} in an interactive mode.
...
the installation, as described in Chapter~\ref{chapter:installation}.
...
In \package{urbansim} datasets are independent from the physical storage of the data. A data storage
...
are stored as an ASCII file in a table format. Alternatively, if the
table is a MySQL \mysqlindex table called ``households'' stored in a
database ``mydatabase,'' you could use

\mysqlindex\index{Storage!mysql}
\begin{verbatim}
>>> import os
...
({\em To do: add an example MySQL file, like household.tab, to play
around with.})

If the mysql database is a scenario database (as described in~\ref{urbansim-database-tables-baseyear-scenario-db}),
replace \verb|opus_database| and \verb|OpusDatabase| in the above code with \verb|scenario_database| and \verb|ScenarioDatabase|.

Opus can support many types of storage formats, including formats you define.
In addition to ``tab'' and ``mysql'', \mysqlindex formats that come with Opus are ``flt''
where the data are stored in a binary format, ``xml'' for xml type of data, ``cvs'' for ASCII comma delimited type
and ``dict'' where the data are passed  directly to the data set class (see
Section \ref{sec:data-storage}).\index{Storage}

Now we can create a household dataset \datasetindex with the \package{urbansim} class \class{HouseholdDataset}, using the created
storage object:
\begin{verbatim}
...
The \class{HouseholdDataset} class is a child class of \class{Dataset} from the
module \\\module{opus_core.datasets.dataset}, and thus it can use any of \class{Dataset}'s
methods (see Sections~\ref{sec:urbansim-datasets} and~\ref{sec:opus-core-datasets}).
...
\package{urbansim} contains many pre-defined dataset \datasetindex classes, such as \class{GridcellDataset},
  ...
  Table~\ref{tab:urbansim-datasets}.
  ...
  Section~\ref{sec:variable-names}). Each dataset \datasetindex defined in \package{urbansim}
   initiates an object of class
  \class{Dataset} by passing default values, such as a dataset \datasetindex name, name of the
  unique identifier or a table name and storage of the input data.
...
\end{verbatim}
\begin{verbatim}
Estimating Choice Model (from opus_core.choice_model):
                                started on Mon Mar 19 19:00:01 2007
    submodel: -2
    Convergence achieved.
    Number of Iterations:  144
    ***********************************************
    Log-likelihood is:            -9.07119837282
    Null Log-likelihood is:       -10.9861228867
    Likelihood ratio index:       0.174303940855
    Adj. likelihood ratio index:  -0.189791749796
    Number of observations:       10
    Suggested |t-value| >         1.51742712939
    Convergence statistic is:     0.000990051660535
    -----------------------------------------------
...
Estimating Choice Model (from opus_core.choice_model): completed.....0.4 sec
\end{verbatim}
...
Running Choice Model (from opus_core.choice_model):
                                     started on Mon Mar 19 19:11:23 2007
    Total number of individuals: 10
    ChoiceM chunk 1 out of 1.: started on Mon Mar 19 19:11:23 2007
        Number of agents in this chunk: 10
    ChoiceM chunk 1 out of 1.: completed.............................0.1 sec
Running Choice Model (from opus_core.choice_model): completed........0.4 sec
>>> choices
array([2, 1, 1, 1, 1, 1, 2, 2, 1, 2])
\end{verbatim}
...
9        800    2
\end{verbatim}
One can load this dataset using the class \class{GridcellDataset} which takes the
same arguments as \class{HouseholdDataset}:
...
\end{verbatim}
The household location choice model (HLCM) creator returns an instance of class
\class{AgentLocationChoiceModel} with some useful default settings for models where agents are households and
locations are gridcells.  The argument \verb|sampler| specifies a module to be
...
and the set of agents for which the model runs.  The optional argument
\verb|debuglevel| controls the amount of outputs during the computation.
\coefficientsindex
\begin{verbatim}
>>> seed(1)
...
(see Section~\ref{sec:choices}) and have a method \method{run()}. The
argument \verb|run_config| is expected to be of type \class{Resources} and
contains parameters for the simulation. These are in our example the name of
...
and how many times the households can re-decide if there is an overflow.
...
\package{urbansim} implements several location choice models and their creators.
...
value of the \class{RegressionModel} constructor and thus, it can be omitted.
...
%
Two specific child classes of the \class{RegressionModel} class are implemented in
\package{urbansim},  namely \class{LandPriceModel} and \class{ResidentialLandShareModel}.
Both models should
...
accessed from the \method{compute()} method by \method{self.get_dataset()}.
...
\class{Dataset} in \package{opus_core.variables}, called
...
\subsubsection{Variable Names in Expressions}
...
expressions given to \method{compute_variables}.  The value of the new
alias is returned as the value of the expression if it's the last item on
the list of variables and expressions.

\subsubsection{Interaction Sets}
...
\subsubsection{Aggregation and Disaggregation}
...
The attribute being aggregated or disaggregated should be either
dataset-qualified or fully-qualified, i.e.\ always including the dataset
name and optionally including the package name.  The result of an
aggregation or disaggregation can be used in more complex expressions,
e.g. \code{ln(2*aggregate(gridcell.population))}.  However, there is
currently a limitation that the attribute being aggregated or disaggregated
must be designated with a single attribute name; it can't be an
expression. (If you want an expression for the first argument, declare a
new variable for it.)  For example, this expression isn't legal currently:

\begin{verbatim}
zone.aggregate(2*gridcell.capacity)
\end{verbatim}

Instead you should define a new variable \code{double_capacity} for
gridcell, and then write

\begin{verbatim}
zone.aggregate(gridcell.double_capacity)
\end{verbatim}
