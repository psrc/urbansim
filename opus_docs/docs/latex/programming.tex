% $Id: programming.tex,v 1.58 2007/06/01 23:49:22 borning Exp $
...
By convention, files installed in the default Python location
(\file{site-packages}) should not be edited. If you need to modify the contents
of \emph{any} of the installed Opus packages, follow the instructions at
\url{http://www.urbansim.org/opus/opus_manual/docs/developer_setup.html} to
create a ``developer setup'' for editing those files.
...
datasets, define new datasets, define models, etc.  These files should not be
put into Python’s \file{site-packages} directory (where Opus is installed by
default), since they would be over-written when you upgrade Opus or UrbanSim.

Instead, we recommend that you create a workspace, e.g.
\file{C:\textbackslash myworkspace} into which you create your own
Opus package(s) to contain these files.  For instance, to create an
Opus package for the fictitious ``atlantis'' MPO:

\begin{enumerate}
  \item Create a directory into which to place your Opus package, e.g.
  \file{C:/myworkspace}.
...
>>> create_package('c:/myworkspace', 'atlantis')
\end{verbatim}
\end{enumerate}

This will create a new directory, e.g. \file{C:/myworkspace/atlantis},
...
\item[\file{.cvsignore}] \index{.cvsignore@\file{.cvsignore}} Informs the CVS
version control system  of files to not commit to the repository (see
\url{http://www.nongnu.org/cvs/}).

\item[\file{.project}] \index{.project@\file{.project}} Eclipse's \eclipseindex
container for project-specific information (see \url{http://www.eclipse.org/}).

\item[\file{build.xml} and \file{fireman.xml}]
\index{build.xml@\file{build.xml}}\index{fireman.xml@\file{fireman.xml}}\firemanindex
Directives for CUSPA's \cuspaindex automated build system, which we call
``Fireman'' (see \url{http://trondheim.cs.washington.edu/fireman/}).
...
\index{Environment variables!MYSQLSTOCHASTICTESTLOGGER@\verb|MYSQLSTOCHASTICTESTLOGGER|}
\index{MYSQLSTOCHASTICTESTLOGGER@\verb|MYSQLSTOCHASTICTESTLOGGER|}
...
\section{Building an Opus Installer}\index{installer!creating}
...
command-line. This will produce \code{..\textbackslash ..\textbackslash
dist\textbackslash opus-major.minor.build.zip}, where \verb|major| is the 
...
\verb|opus_version| property in \package{nightly}'s
\file{build.xml} file.  Each
package specifies its own version number in the \verb|version| property within
its \file{opus_package_info.py} file.
