% $Id: using-urbansim.tex,v 1.96 2007/06/01 23:49:22 borning Exp $
...
\emph{15 Sept 2006: Note: we are in the process of splitting the following
configuration information into separate parts.  Once we are done
with that refactoring, we will update the documentation to the new arrangement.}
...
The \verb|'cache_mysql_data'| entry is the full Opus path to the class to use to
create a baseyear cache from the baseyear data in the MySQL database.  The
\verb|'urbansim.model_coordinators.cache_mysql_data'| version creates  both the
...
objects that \class{ModelSystem} provides are \verb|mysql_storage|
...
\verb|gui_export_cache_to_mysql_database.py| tool located in
