...
class Households2010Model(HouseholdLocationChoiceModel):
    ...
    def run(self, specification, coefficients, agent_set, agents_index=None, **kwargs):
        ...
        household_zonetype = agent_set.compute_variables("((household.zone_id * 100) + ((household.building_type_id)*(household.building_type_id==1) + (2)*(household.building_type_id==2) + (2)*(household.building_type_id==3) + (household.building_type_id)*(household.building_type_id>3) )).astype('i4')")
        
        building_zonetype = building_set.compute_variables("((building.zone_id * 100) + ((building.building_type_id)*(building.building_type_id==1) + (2)*(building.building_type_id==2) + (2)*(building.building_type_id==3) + (building.building_type_id)*(building.building_type_id>3) )).astype('i4')")
        ...
        household_county = agent_set.compute_variables("(household.disaggregate(zone.aggregate(parcel.county_id,function=median))).astype('i4')")
