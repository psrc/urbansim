...
class HouseholdFastAllocationModel(Model):
    """
    """
    model_name = "Allocating households randomly to residential building within submarket"
        ...
        for submarket in unique_chosen_submarkets:
            if submarket > 0:
                ...
                if num_buildings>0:
                    ...
                    ###NEXT STEP:  NEED TO ACCOUNT FOR BUILDING CAPACITY!!
        ...
        # for zone_id in zone_ids:
            ...
            # for job in idx_unplaced_jobs:
                ...
                # if idx_available_job_capacity.size < 1:
                    # logger.log_status("No non-residential job capacity (based on square footage) remaining in zone %s"%(zone_id))
                    # building_in_zone = building_set.compute_variables('(building.disaggregate(parcel.zone_id)==%s)*(building.building_type_id>2)'%(zone_id))
                    ...
                    # if idx_building.size < 1:
                        ...
                        # if idx_building.size < 1:
                            ...
                            # building_to_add = {}
                            # building_to_add['building_type_id'] = array([5])
