...
class Buildings2010TransitionModel(Model):
    ...
    def run(self):
        ...
        building_set.add_primary_attribute(name='building_zone_region', data=building_set.compute_variables('building.disaggregate(parcel.county_id)'))
        building_set.add_primary_attribute(name='residential_building', data=building_set.compute_variables('building.building_type_id<3'))
        ...
        for zone_id in zone_ids:
            ...
            index_new_mf_units = where(logical_and(building_set['building_zone']==zone_id, (building_set['building_type_id']==2)*(building_set['parcel_id']==-1)))[0]
            if index_new_sf_units.size > 0:
                for building in index_new_sf_units:
                    ...
                    has_available_sf_capacity = parcel_set.compute_variables('_has_available_sf_capacity = (parcel.zone_id==%s) *  ((parcel.parcel_acres*10).round().astype(int32)) - (parcel.number_of_agents(building)))>0)'%(zone_id))
            ...
            if index_new_mf_units.size > 0:
                for building in index_new_mf_units:
                    ...
                    available_mf_capacity = parcel_set.compute_variables('_available_mf_capacity = (parcel.zone_id==%s) *  ((parcel.parcel_acres*30).round().astype(int32)) - (parcel.aggregate(building.residential_units)))>0)'%(zone_id))
