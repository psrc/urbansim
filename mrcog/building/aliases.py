...
aliases = [
    ...
    "residential_building_type_id = 1*((building.residential_units>0)*(building.building_type_id==1)) + 2*((building.residential_units>0)*(building.building_type_id==2)) +3*(building.residential_units==0)",
    "number_of_non_home_based_jobs = building.aggregate((job.home_based_status==0))",
    "occupied_spaces=urbansim_parcel.building.is_residential * urbansim_parcel.building.number_of_households + numpy.logical_not(urbansim_parcel.building.is_residential) * mrcog.building.number_of_non_home_based_jobs *urbansim_parcel.building.building_sqft_per_job",
    ...
    "total_job_spaces = safe_array_divide(building.non_residential_sqft, mrcog.building.building_sqft_per_employee)",
    "vacant_job_spaces = clip_to_zero(mrcog.building.total_job_spaces - mrcog.building.employees)",
    "employees=building.aggregate((job.job_id>0))",
