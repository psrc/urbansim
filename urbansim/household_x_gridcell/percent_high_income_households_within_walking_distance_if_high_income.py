...
class percent_high_income_households_within_walking_distance_if_high_income(Variable):
    """Percent of households within the walking radius that are designated as high-income, given that the decision-making household is high-income.
    [percent_high_income_households_within_walking_distance if hh.is_high_income is true else 0]"""    
    
    gc_percent_high_income_households_within_walking_distance = \
      "percent_high_income_households_within_walking_distance"
    hh_is_high_income = "is_high_income"
        
    def dependencies(self):
        return [attribute_label("gridcell", self.gc_percent_high_income_households_within_walking_distance), 
                attribute_label("household", self.hh_is_high_income)]
