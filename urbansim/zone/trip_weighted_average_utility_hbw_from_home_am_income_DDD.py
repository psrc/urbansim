...
class Tests(opus_unittest.OpusTestCase):
    ...
    variable_name = "urbansim.zone.trip_weighted_average_utility_hbw_from_home_am_income_1"
        ...
        values = VariableTestToolbox().compute_variable(self.variable_name, 
            {"zone":{
             ...
             "travel_data":{ 
                 ...
                 "trips_hbw_am_income_1":array([1.1,2.1,0.0, 0.0]), 
                 "trips_hbw_am_income_2":array([4.1,3.1,2.1,1.1])}}, 
            dataset = "zone")
        should_be = array([(1.1 * -4.8+2.1*-2.222)/(1.1+2.1), 
                           (1.1 * -4.8+2.1*-2.222)/(1.1+2.1)])# the last one should automatically be the first (div 0)
