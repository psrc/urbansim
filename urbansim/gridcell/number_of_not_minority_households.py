...
class number_of_not_minority_households(Variable):
    """Number of not minority households in a gridcell"""

    _return_type="int32"
    is_not_minority = "is_not_minority"

    def dependencies(self):
        return [attribute_label("household", self.is_not_minority), 
                attribute_label("household", "grid_id"), 
                my_attribute_label("grid_id")]

    def compute(self, dataset_pool):
        households = dataset_pool.get_dataset('household')
        return self.get_dataset().sum_dataset_over_ids(households, self.is_not_minority)
