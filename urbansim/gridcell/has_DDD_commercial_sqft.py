...
class has_DDD_commercial_sqft(Variable):
    ...
    def compute(self, dataset_pool):
        return self.get_dataset().get_attribute(self.commercial_sqft) == self.tnumber
    ...
    def test_my_inputs(self):
        tester = VariableTester(
            ...
            test_data={
                "gridcell":{ 
                    "grid_id":array([1, 2, 3]),
                    "commercial_sqft":array([1, 2, 5])
                }
            }
        )

        should_be = array([False, True, False])
        instance_name = "urbansim.gridcell.has_2_commercial_sqft"        
