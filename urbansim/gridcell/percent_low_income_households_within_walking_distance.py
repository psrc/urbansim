...
class percent_low_income_households_within_walking_distance(Variable):
    """Percent of households within the walking radius that are designated as low-income.
    [100 * (sum (over c in cell.entity_within_walking_radius) of (count of households hh placed in c such that hh.is_low_income)) /
    (sum (over c in cell.entity_within_walking_radius) of (count of households hh placed in c))"""
    _return_type="float32"
    number_of_low_income_households_within_walking_distance = "number_of_low_income_households_within_walking_distance"
    ...
    def compute(self, dataset_pool):
        ...
        return 100.0*ma.filled(self.get_dataset().get_attribute(self.number_of_low_income_households_within_walking_distance)/ 
...
class Tests(opus_unittest.OpusTestCase):
    def test_my_inputs(self):
        number_of_low_income_households_within_walking_distance = array([0, 333, 500, 100])
