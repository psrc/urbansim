...
class average_income_within_walking_distance(Variable):
    """Average income within walking distance, computed by dividing the
    total income within walking distance by the number of household"""

    def dependencies(self):
        return [my_attribute_label("sum_income_within_walking_distance"),
    ...
    def pre_check(self, dataset_pool):
        ...
        values = self.get_dataset().get_attribute("sum_income_within_walking_distance")[index]
        self.do_check("x == 0", values)

    def compute(self, dataset_pool):
        hhs_wwd = self.get_dataset().get_attribute("number_of_households_within_walking_distance")
        return ma.filled(self.get_dataset().get_attribute("sum_income_within_walking_distance")/
...
class Tests(opus_unittest.OpusTestCase):
    def test_my_inputs(self):
        sum_income_within_walking_distance = array([0, 5000, 10000, 15000])
        ...
        tester = VariableTester(
            ...
            test_data={
                "gridcell":{
                    ...
                    "sum_income_within_walking_distance":sum_income_within_walking_distance,
