...
#           300 households of age > 50 and income >= 40,000
    ...
    def test_do_nothing_if_no_agents(self):
        ...
        storage._write_dataset(out_table_name='rates', values=self.annual_relocation_rates_for_households_data)            
    ...
    def test_hrm_all_households_relocate_with_100_percent_probability(self):
        ...
        storage._write_dataset(out_table_name='households', values=self.household_data)
        hh_set = HouseholdDataset(in_storage=storage, in_table_name='households')
    ...
    def test_hrm_no_households_relocate_with_0_percent_probability(self):
        ...
        storage._write_dataset(out_table_name='households', values=self.household_data)
        storage._write_dataset(out_table_name='rates', values = annual_relocation_rates_for_households_data)
    ...
    def test_erm_correct_distribution_of_jobs_relocate(self):
        ...
        storage._write_dataset(out_table_name='rates',
            values = {
                'sector_id':array([1,2]), 
                'job_relocation_probability':array([0,0.5]),
                }
            )
        rates = RateDataset(in_storage=storage, in_table_name='rates', what='jobs')
        
        # run model
        model = EmploymentRelocationModelCreator().get_model(debuglevel=0)
        hrm_resources = Resources({"rate":rates})
    ...
    def test_erm_proportion_jobs_relocate_with_varied_percent_probability(self):
        ...
        jobs_rateset = RateDataset(in_storage=storage, in_table_name='rates', what='jobs')

        erm_resources = Resources({"rate":jobs_rateset})
