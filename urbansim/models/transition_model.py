...
class TransitionModel(Model):
    ...
    def run(self, year=None, target_attribute_name='number_of_households', dataset_filter="", reset_dataset_attribute_value={}, dataset_pool=None,  **kwargs):
        """ dataset_filter attribute/variable indicates which records in the dataset are eligible in the sampling for removal or cloning
        """
        ...
        if dataset_filter:
            short_name = VariableName(dataset_filter).get_alias()
            if short_name not in dataset_known_attributes:
                filter_indicator = self.dataset.compute_variables(dataset_filter, dataset_pool=dataset_pool)
            else:
        ...
        for index in range(control_totals_for_this_year.size()):
            ...
            if actual_num != target_num:
                lucky_index = probsample_noreplace(arange(self.dataset.size()),
                                                 abs(actual_num - target_num),
                                                 prob_array=(indicator*filter_indicator).astype(float64)/indicator.sum())
                if actual_num < target_num:
