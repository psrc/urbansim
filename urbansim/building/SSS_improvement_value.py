...
class SSS_improvement_value(Variable):
    """improvement_value of given type."""
    ...
    def dependencies(self):
        return [my_attribute_label(self.is_type_variable), 
                my_attribute_label("improvement_value")]

    def compute(self, dataset_pool):
        buildings = self.get_dataset()
        iv = buildings.get_attribute("improvement_value")
        return where(buildings.get_attribute(self.is_type_variable), iv, 0)
...
class Tests(opus_unittest.OpusTestCase):
    variable_name = "urbansim.building.commercial_improvement_value"
        ...
        values = VariableTestToolbox().compute_variable(self.variable_name,
            data_dictionary = {
                'building': {
                    ...
                    'improvement_value': array([100, 350, 1000, 0, 430, 95])
                    },
