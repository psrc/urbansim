...
class workerDDD_employment_of_group_retail_accessible_from_work_to_home_drive_alone(abstract_time_space_prism_variable):
    ...
    def __init__(self, worker):
        self.agent_resource = "household.aggregate((psrc.person.worker%s==1).astype(int32)*(person.disaggregate(tour.arr_time - tour.dep_time - 15)))" % worker
        self.agent_zone_id = "worker%s_workplace_zone_id = household.aggregate((psrc.person.worker%s == 1) * urbansim_parcel.person.workplace_zone_id ).astype(int32)" % (worker, worker)
