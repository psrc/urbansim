...
aliases = [
          ...
          #  This is the actual term called in the Project XML file - two above are defining variables used below
            "units_proposed_plus_minimum_1DU_per_legal_lot_yield = %s + numpy.logical_and(%s, numpy.logical_or(%s,%s))" % (
             ...
             "total_investment = (development_project_proposal.disaggregate((parcel.aggregate((psrc_parcel.development_project_proposal.total_revenue - urbansim_parcel.development_project_proposal.construction_cost), function=maximum)))*.91+ urbansim_parcel.development_project_proposal.construction_cost - (development_project_proposal.disaggregate(urbansim_parcel.parcel.improvement_value) * (psrc_parcel.development_project_proposal.is_redevelopment == 0))).astype(float32)",
