...
class max_network_distance_from_home_to_work(Variable):
    ...
    def dependencies(self):
        return [ 
                "psrc_parcel.household.worker1_network_distance_from_home_to_work",
                "psrc_parcel.household.worker2_network_distance_from_home_to_work",
             ]

    def compute(self, dataset_pool):
        dataset = self.get_dataset()
        data1 = dataset.get_attribute("worker1_network_distance_from_home_to_work")
        data2 = dataset.get_attribute("worker2_network_distance_from_home_to_work")
    ...
    def test_my_inputs(self):
        tester = VariableTester(
            ...
            'travel_data': {
                ...
                'single_vehicle_to_work_travel_distance':array([1.1, 2.2, 3.3, 4.4, 6.5, 0.7, 8.7, 7.8, 1.0])
            }
        })
